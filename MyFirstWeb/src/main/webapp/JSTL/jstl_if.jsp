...
<body>
	<c:if test="${param.name == '홍길동' }"> <%--==은 eq, !=는 ne --%>
		<p>name 파라미터로 들어온 값은 ${param.name }입니다.</p><br/>
		<%--c태그라이브러리의 out을 사용하면 value="출력할 값" 형태로 out.println()등을 대체합니다. --%>
		<c:out value="name 파라미터로 들어온 값은 ${ param.name }입니다."></c:out>
	</c:if>
	<!-- name 파라미터에 이순신이 들어오면 name파라미터로 들어온 값은 이순신 입니다.
	라고 출력하도록 아래쪽에 직접 한 번 작성해보세요. -->
	<c:if test="${param.name == '이순신' }">
		<p>name 파라미터로 들어온 값은 ${param.name }입니다.<br/>
		<c:out value="name 파라미터로 들어온 값은 ${ param.name }입니다."></c:out>
	</c:if>
